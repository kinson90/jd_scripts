...
一次性活动，运行完脚本获徿53京豆，进入入口还可以开卡领30郿
*/
const $ = new Env('开卡活势');
...
!(async () => {
  $.newShareCodes = []
  if (!cookiesArr[0]) {
    $.msg($.name, '【提示】请先获取京东账号一cookie\n直接使用NobyDa的京东签到获叿', 'https://bean.m.jd.com/', {"open-url": "https://bean.m.jd.com/"});
    return;
  }
  for (let j=0;j<configList.length; ++j) {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
      if (cookiesArr[i]) {
        ...
        message = '';
        await TotalBean();
        console.log(`\n******开始【京东账叿${$.index}?${$.nickName || $.UserName}*********\n`);
        if (!$.isLogin) {
          $.msg($.name, `【提示】cookie已失效`, `京东账号${$.index} ${$.nickName || $.UserName}\n请重新登录获取\nhttps://bean.m.jd.com/`, {"open-url": "https://bean.m.jd.com/"});

          if ($.isNode()) {
            await notify.sendNotify(`${$.name}cookie已失敿 - ${$.UserName}`, `京东账号${$.index} ${$.UserName}\n请重新登录获取cookie`);
          } else {
            ...
            if(data['success']){
              for (let i = 0; i < data.data.myTasks.length; ++i){
                ...
                if(task.groupType===1) {
                  ...
                  $.log(`关注店铺任务，完成次敿: ${task.finishCount}/${task.itemCount}`)
                } else if(task.groupType===2){
                  groupType = 'ADD_CART'
                  $.log(`加购任务，完成次敿: ${task.finishCount}/${task.itemCount}`)
                  ...
                  $.log(`浏览商品任务，完成次敿: ${task.finishCount}/${task.itemCount}`)
                }
              ...
              if (data.data.rewardStatus!==3) {
                $.log(`去领取额夿${data.data.rewardFinish}京豆`)
                await doTask("FINISH_TASK",0)
                const res = await doTask("FINISH_TASK",0)
                if(res['success']){
                  $.log(`领取额外${data.data.rewardFinish}京豆完成成功，获徿${data.data.rewardFinish}京豆`)
                }
              }
            } else{
              console.log('获取失败，请检查是否黑叿')
            }
          }
        }
      } catch (e) {
